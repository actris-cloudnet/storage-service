...
export class DB {
  ...
  async saveObject(bucket: string, bucketId: number, key: string) {
    return Promise.all([
      this.queryOne(`INSERT INTO ${bucket} (key, bucket_id) VALUES ($1, $2)`, [
        key,
        bucketId,
      ]),
      this.queryOne(
  ...
  async deleteObject(bucket: string, key: string) {
    return Promise.all([
      this.queryOne(`DELETE FROM ${bucket} WHERE key = $1`, [key]),
      this.queryOne(
        `UPDATE bucketstats
         SET n_objects = n_objects - 1
         WHERE bucket = $1`,
  ...
  async selectBucket(bucket: string) {
    const { table } = await this.queryOne("SELECT to_regclass($1) as table", [
      bucket,
    ]);
    return table;
  }
